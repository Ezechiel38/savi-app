...
Symptômes: ${symptoms}`;
  ...
  try {
    ...
    if (data.error) {
      ...
      return res.status(500).json({ error: "Erreur API: " + (data.error.message || JSON.stringify(data.error)) });
    }
    ...
    return res.status(500).json({ error: "Erreur: " + (error.message || "inconnue") });
  }
}
